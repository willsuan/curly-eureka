...
// f(x0,x1) = lbeta(x0+1.2, x1+1.1) + sin(x0*x1)
struct F {
  template <class Num>
  CDUAL_HD Num operator()(const std::array<Num,2>& x) const {
    return lbeta(x[0]+Num(1.2), x[1]+Num(1.1)) + sin(x[0]*x[1]);
  }
  double host_eval(const double* v) const {
    return std::lgamma(v[0]+1.2) + std::lgamma(v[1]+1.1) - std::lgamma(v[0]+v[1]+2.3) + std::sin(v[0]*v[1]);
  }
};

int main(){
  using T=double; constexpr int N=2; const int batch=1; T x[N]={0.3, 0.8};
  ...
  std::printf("lbeta: grad err=%.3e, hess err=%.3e\n", ge, he);
  bool ok=(ge<5e-7)&&(he<5e-5); std::printf("RESULT: %s\n", ok? "OK":"FAIL");
